...
ex. `python 07_part1.py < day_seven_input.txt` 
"""
...
def get_hand_strength(hand: list) -> int:
    ...
    # get counts of each unique card we have and sort by what we have the most ex. [4, 2], [2, 2, 1], [1, 1, 1, 1, 1]
    card_counts = sorted(reduce(lambda combined, c: {**combined, c: (combined.get(c) or 0) + 1}, hand, {}).values(), reverse=True)

    # get hand strength
    strength = (
        7 if card_counts[0] == 5 else                           # five of a kind (5)
        6 if card_counts[0] == 4 else                           # four of a kind (4, 1)
        5 if card_counts[0] == 3 and card_counts[1] == 2 else   # full house (3, 2)
        4 if card_counts[0] == 3 else                           # three of a kind (3,)
        3 if card_counts[0] == 2 and card_counts[1] == 2 else   # two pairs (2, 2, 1)
        2 if card_counts[0] == 2 else                           # two of a kind (2,)
